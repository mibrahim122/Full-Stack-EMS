...
//     setLoading(true)
//     try {
//       const queryParam = selectedDept ? `?department=${selectedDept}` : ''
//       const response = await api.get(`/employees${queryParam}`)
...
const Employees = () => {
  ...
  // UPDATED FILTER: Excludes any employee whose isDeleted flag is true
  const filtered = employees.filter((emp) => {
    if (emp.isDeleted) return false; 
    return `${emp.firstName} ${emp.lastName} ${emp.position}`.toLowerCase().includes(search.toLowerCase());
  })
